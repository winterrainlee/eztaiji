...
상단의 과정·자세를 누르면 `舒筋十二式` 또는 `易簡太極拳 · 1단`과 자세를 선택할 수 있습니다. ‘이 자세 시작’을 누르면 0동작부터 봅니다. 舒筋十二式의 0동작은 현장 자료의 `預備式`이고, 易簡太極拳의 0동작은 자세의 시작 상태입니다. 같은 과정 안에서는 하단의 이전·다음으로 이동합니다. 별도의 [소개 페이지](https://winterrainlee.github.io/eztaiji/about.html)에서 개요·구성·참고 자료를 확인합니다.

舒筋十二式은 2026-09-25 國立東華大學 태극권 동아리에서 직접 촬영한 `太極拳舒筋十二式` 교육자료를 기준으로 12식·38개 세부 동작을 원문 순서대로 제공합니다. 현장 원문은 [`docs/sources/donghwa-shujin12.md`](docs/sources/donghwa-shujin12.md)에 별도로 보존합니다. 좌표판은 전 과정에 일괄 적용하지 않고, 원문에서 중심 이동·한쪽 다리 들기·전방 차기/蹬出이 확인되는 `下蹲式`·`抖腕抖跗`·`踢腿`·`蹬腿`의 해당 단계에만 선택적으로 표시합니다. 발 위치의 수치 좌표와 허실 표기는 학습용 근사·프로젝트 해석으로 원문 지시와 구분합니다.
