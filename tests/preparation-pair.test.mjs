...
test('next start shares final state but not events or previous explanations',()=>{const x=fresh();addNext(x);const d=compileData(x);assert.equal(d.views['test-next-start-view'].stateId,end);assert.deepEqual(d.views['test-next-start-view'].events,[]);assert.deepEqual(d.views['test-next-start-view'].principleIds,[]);assert.equal(d.views['test-next-start-view'].interpretationIds.includes('p002-m06-principle'),false);});
...
test('older collection data without global-principle fields still compiles',()=>{const x=fresh();delete x.principles;for(const m of Object.values(x.motions))delete m.principleIds;const d=compileData(x);assert.deepEqual(d.principles,{});assert.deepEqual(d.views['p002-m03-view'].principleIds,[]);});
